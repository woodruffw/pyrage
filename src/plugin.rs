...
impl PyCallbacks {
    fn new(inner: Py<PyAny>, py: Python<'_>) -> PyResult<Self> {
        {
            let inner = inner.as_ref(py);

            for method_name in [
                "display_message",
                "confirm",
                "request_public_string",
                "request_passphrase",
            ] {
                // I would use `hasattr`, but it doesn't throw, and
                // constructing pretty AttributeErrors is incredibly
                // hard.
                //
                // XXX: Should we even throw it? Better ask for
                // forgiveness than permission, and we're panicking
                // when things get called. And static typecheckers
                // will catch a protocol violation.
                inner.getattr(method_name)?;
            }
        }
        Ok(Self(inner))
    }
...
impl Identity {
    ...
    #[classmethod]
    fn default_for_plugin(_cls: &PyType, plugin: &str) -> Self {
