use std::str::FromStr;

use pyo3::{prelude::*, types::PyType};

use crate::RecipientError;

#[pyclass(from_py_object, module = "pyrage.tagpq")]
#[derive(Clone)]
pub(crate) struct Recipient(pub(crate) age::tagpq::Recipient);

#[pymethods]
impl Recipient {
    #[classmethod]
    fn from_str(_cls: &Bound<'_, PyType>, v: &str) -> PyResult<Self> {
        age::tagpq::Recipient::from_str(v)
            .map(Self)
            .map_err(RecipientError::new_err)
    }

    fn __str__(&self) -> String {
        self.0.to_string()
    }
}

pub(crate) fn module(py: Python<'_>) -> PyResult<Bound<'_, PyModule>> {
    let module = PyModule::new(py, "tagpq")?;

    module.add_class::<Recipient>()?;

    Ok(module)
}
